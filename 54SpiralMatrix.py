# -*- coding: utf-8 -*-
"""
Created on Sat Nov  3 21:56:47 2018
@author : HaiyanJiang
@email  : jianghaiyan.cn@gmail.com

54. Spiral Matrix  螺旋矩阵
Input:
[
 [ 1, 2, 3 ],
 [ 4, 5, 6 ],
 [ 7, 8, 9 ]
]
Output: [1,2,3,6,9,8,7,4,5]

Approach 1: Simulation
Intuition
Draw the path that the spiral makes. 模拟出它的行走路径
We know that the path should turn clockwise whenever it would go out of bounds
or into a cell that was previously visited.
"""


class Solution(object):
    def spiralOrder(self, matrix):
        if not matrix:
            return []
        R, C = len(matrix), len(matrix[0])
        dr = [0, 1, 0, -1]  # same row numb, row numb+1, same, row numb-1
        dc = [1, 0, -1, 0]
        seen = [[False] * C for _ in matrix]
        ans = []
        r, c, di = 0, 0, 0
        for _ in range(R * C):
            ans.append(matrix[r][c])
            seen[r][c] = True
            # get the current row and current column
            cr, cc = r + dr[di], c + dc[di]
            if 0 <= cr < R and 0 <= cc < C and not seen[cr][cc]:
                r, c = cr, cc
            else:
                di = (di + 1) % 4
                r, c = r + dr[di], c + dc[di]
        return ans


matrix = [[]]
matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
sol = Solution()
sol.spiralOrder(matrix)


class Solution2(object):
    def spiralOrder(self, matrix):
        def spiral_coords(r1, c1, r2, c2):
            for c in range(c1, c2 + 1):
                yield r1, c
            for r in range(r1 + 1, r2 + 1):
                yield r, c2
            if r1 < r2 and c1 < c2:
                for c in range(c2 - 1, c1, -1):
                    yield r2, c
                for r in range(r2, r1, -1):
                    yield r, c1

        if not matrix:
            return []
        ans = []
        r1, r2 = 0, len(matrix) - 1
        c1, c2 = 0, len(matrix[0]) - 1
        while r1 <= r2 and c1 <= c2:
            for r, c in spiral_coords(r1, c1, r2, c2):
                ans.append(matrix[r][c])
            r1 += 1
            c1 += 1
            r2 -= 1
            c2 -= 1
        return ans


"""
g = spiral_coords(0, 0, 3, 3)
for i, j in g:
    print(i, j)
"""


matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]

sol2 = Solution2()
sol2.spiralOrder(matrix)

sol = Solution()
sol.spiralOrder(matrix)

